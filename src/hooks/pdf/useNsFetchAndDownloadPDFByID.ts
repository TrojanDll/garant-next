...
import { IGetNsPDFByIdRequest, IGetOsagoPDFByIdRequest } from "@/types/pdf.types";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import axios from "axios";
import { useCallback } from "react";

export function useNsFetchAndDownloadPDFById() {
  ...
  const { mutate, isPending, data, isSuccess, isError } = useMutation({
    mutationKey: ["nsPDFById"],
    mutationFn: (data: IGetNsPDFByIdRequest) => pdfService.getNsPDFById(data),
